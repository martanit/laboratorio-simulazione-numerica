...
int main() {
  ...
  std::ifstream in("random/seed.in");
  if (in.is_open()) {
    while (!in.eof()) {
...
void set_parameters(void) {
  ...
  std::cout << "Boltzmann weight exp(- beta * H ), beta = 1/T " << std::endl
            << std::endl;
  std::cout << "The program uses k_B=1 and mu_B=1 units " << std::endl;
  // Read input informations
  ReadInput.open("input.dat");
  ReadInput >> temp >> nspin >> J >> h >> metro // 1=metropolis
      >> nblk >> nstep;
  beta = 1.0 / temp;
...
void measure() {
  ...
  walker = {H / double(nspin),
            ...
            beta * std::pow(m, 2) / (double)nspin, m / (double)nspin};
}
...
void observable_temp(double t) {
  ...
  if (metro == 1) {
    if (h == 0) {
      ...
      u_out << t << " " << glob_avg[0] / (double)nblk << " " << error[0]
            << std::endl;

      h_out.open("../data/heat_temp_metro.out", std::ios::app);
      h_out << t << " " << glob_avg[1] / (double)nblk << " " << error[1]
            << std::endl;

      chi_out.open("../data/chi_temp_metro.out", std::ios::app);
      chi_out << t << " " << glob_avg[2] / (double)nblk << " " << error[2]
              << std::endl;
    } else if (h != 0) {
      m_out.open("../data/magnetization_temp_metro.out", std::ios::app);
      m_out << t << " " << glob_avg[3] / (double)nblk << " " << error[3]
            << std::endl;
    }
  } else {
    if (h == 0) {
