...
constants.h: contains the declaration of every global variable that has been
used. functions.h: contains the declaration of the function with a brief
description. Once compiled, QMC1D is invoked with the command: "./qmc1d". It
will read the settings in the file "input.dat".
*/
#include "constants.h"
#include "functions.h"
...
double external_potential(double val) {
  return std::pow(val,2)*(std::pow(val,2)-5./2.);
}
...
double variationalWaveFunction(double v) {
  ...
  double mu = 0.795;
  double sigma = 0.629; 
  return std::exp(-std::pow(v - mu, 2) / (2. * std::pow(sigma, 2))) +
         std::exp(-std::pow(v + mu, 2) / (2. * std::pow(sigma, 2)));
}
...
void translation() {
  ...
  for (int i = 0; i < last; i++) {
    ...
    newcorr = potential_density_matrix(positions[i] + delta,
                                       positions[inext] + delta);
    oldcorr = potential_density_matrix(positions[i], positions[inext]);
    acc_density_matrix_difference += oldcorr - newcorr;
  }
  // metropolis: PIGS contains also the statistical weight of the variational
  // Wave Function.
  double acceptance_probability = exp(-acc_density_matrix_difference);

  if (PIGS)
    acceptance_probability *=
        variationalWaveFunction(positions[0] + delta) *
...
/* BB removes a segment of the polymer, in this case from "starting_point+1" to
"endpoint-1" and replaces it with a free particle propagation. The free particle
propagation is achieved with the gaussian sampling of the kinetic part of the
density matrix. The function index_mask handles the compatibility between PIGS
and PIMC: in PIGS the polymer is open, so you can't have a starting index
greater than an ending index. In PIMC, instead, you have a ring polymer so when
you reach the end you can continue from the beginning. The compatibility
solution that has been chosen consists in viewing the ring polymer as an open
polymer that has been closed on periodic boundary contitions. index_mask takes
this into account. */
void brownianBridge() {
  totalBB++;
  int available_starting_points =
      timeslices - brownianBridgeReconstructions - 1; // for PIGS simulation
  if (!PIGS)
  ...
  for (int i = 0; i < brownianBridgeReconstructions; i++) {
    ...
    double average_position =
        previous_position +
        (ending_coord - previous_position) / (left_reco + 1);
  ...
  // metropolis. Note that the kinetic part has been sampled exactely, thus only
  // the potential part of the density matrix determines the acceptance
...
and replaces it with a free particle propagation using a Brownian Bridge after
the sampling of the starting (left move) or final (right move) position. The
free particle propagation is achieved
...
void brownianMotion(int which) // BM is called only for PIGS simulations
{
  ...
  double starting_coord, ending_coord, average_position, variance, newposition,
      oldposition;
  ...
  for (int i = 0; i < brownianMotionReconstructions; i++) {
    ...
    // gaussian sampling of the free particle propagator
    average_position = previous_position +
                       (ending_coord - previous_position) / (left_reco + 1);
  ...
  for (int i = 0; i < brownianMotionReconstructions + 1; i++) {
    double newcorr, oldcorr;
    newcorr = potential_density_matrix(new_segment[i], new_segment[i + 1]);
    oldcorr = potential_density_matrix(positions[starting_point + i],
                                       positions[starting_point + i + 1]);
    acc_density_matrix_difference += oldcorr - newcorr;
  }

  double acceptance_probability = exp(-acc_density_matrix_difference) *
                                  variationalWaveFunction(newposition) /
                                  variationalWaveFunction(oldposition);
  if (rnd.Rannyu() < acceptance_probability) {
...
void consoleOutput() {
  ...
  cout << "Transl: " << ((double)acceptedTranslations) / totalTranslations
       << endl;
  cout << "BB: " << ((double)acceptedBB) / totalBB << endl;
}

/* This function accumulates the expectation values in their respective
   variables. At the end of the block, these variables are divided by the
   MCSTEPS value and the block average and its squared value are accumulated in
   apposite variables.
 */
...
void upgradeHistogram() {
  ...
  for (int i = timeslices_averages_start; i <= timeslices_averages_end;
       i += 1) {
    int k = 1;
...
/*
finalize**** functions average the accumulators of the block averages and their
square values. Then the error is calculated by the usual formula
...
void finalizePotentialEstimator() {
  ...
  for (int i = 0; i < timeslices; i++) {
    ...
    double potential_energy_square_avg =
        potential_energy_square_accumulator[i] / blocks;
    double p_error =
        sqrt(abs(potential_energy_average * potential_energy_average -
                 potential_energy_square_avg) /
             blocks);
...
void finalizeKineticEstimator() {
  ...
  for (int i = 0; i < timeslices; i++) {
    ...
    double kinetic_energy_square_avg =
        kinetic_energy_square_accumulator[i] / blocks;
    double k_error = sqrt(abs(kinetic_energy_average * kinetic_energy_average -
                              kinetic_energy_square_avg) /
                          blocks);
...
void finalizeHistogram() {
  ...
  for (int i = 0; i < histogram_bins; i++) {
    ...
    out << current_position << " " << hist_average / norma << " "
        << error / norma << endl;
  }
  out.close();
}
...
double kineticEstimator(double value, double next_value) {
  ...
  double term_2 =
      (dtau / 2) * external_potential_second(value) + kinetic_second;
...
void readInput() {
  ...
  input_file >> string_away >> timeslices_averages_start >>
      timeslices_averages_end;
  input_file.close();
  delete[] string_away;
}
