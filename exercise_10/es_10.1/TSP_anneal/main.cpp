...
int main(int argc, char *argv[]) {
  ...
  unsigned int step(0),count(0), nstep(5E5);

  std::vector<double> p = {0.1,0.1, 0.1,0.1};

  std::vector<unsigned int> trial_chromo, 
                            shortest_path;
  ...
  do {
      ...
      if( ((1./fit.path_fitness(trial_chromo))-(1./fit.path_fitness(chromosome))) < 0 ){
        accepted++;
        chromosome = trial_chromo;
      }
      else
      {
        if(rnd.Rannyu() < std::exp(-1.*beta*((1./fit.path_fitness(trial_chromo)-(1./fit.path_fitness(chromosome)))))){
          accepted++;
            chromosome = trial_chromo;
        }
      }
