...
void read_parm(void) {
  ...
  std::cout << "Molecular dynamics simulation in NVE ensemble  " << std::endl
            << std::endl;
  std::cout
      << "Interatomic potential v(r) = 4 * epsilon [(sigma/r)^12 - (sigma/r)^6]"
      << std::endl
      << std::endl;
  ...
  std::cout << "The program integrates Newton equations with the Verlet method "
            << std::endl;
...
void first_move(void) {
  ...
  if (old == 1) {
    ...
    std::cout << "Read initial configuration from file old.0 " << std::endl
              << std::endl;
  ...
  if (old == 1) {
    std::cout << "Calculate velocities from r[t-dt] and r[t+dt]" << std::endl
              << std::endl;
    double xnewv[m_part], ynewv[m_part], znewv[m_part], fx[m_part], fy[m_part],
        fz[m_part];
...
void conf_final(void) {
  ...
  std::ofstream WriteConf;
  std::cout << "Print final configuration to file config.final " << std::endl
            << std::endl;
  ...
  if (old == 1) {
    ...
    std::cout << "Print final configuration to file old.final " << std::endl
              << std::endl;
    WriteConf_old.open("config/old.final");

    for (int i = 0; i < npart; ++i) {
      WriteConf_old << xold[i] / box << "   " << yold[i] / box << "   "
                    << zold[i] / box << std::endl;
    }
    WriteConf_old.close();
  }
...
void conf_xyz(int nconf) { // Write configuration in .xyz format
  ...
  for (int i = 0; i < npart; ++i) {
    WriteXYZ << "LJ  " << pbc(x[i]) << "   " << pbc(y[i]) << "   " << pbc(z[i])
             << std::endl;
  }
  WriteXYZ.close();
}
...
void measure(int istep, int iblock) {
  ...
  if (istep == (nstep / double(nblock))) {
    ...
    // ave_epot << istep+nstep/(nblock*10.)*iblock << " " <<
    // sum_pot/double(iblock+1) << " " << dev_st_mean(iblock+1 , sum_pot,
    // sum2_pot) << std::endl; ave_ekin << istep+nstep/(nblock*10.)*iblock << "
    // "<< sum_kin/double(iblock+1) << " "  << dev_st_mean(iblock+1, sum_kin,
    // sum2_kin) << std::endl; ave_temp << istep+nstep/(nblock*10.)*iblock <<" "
    // << sum_temp/double(iblock+1) << " " << dev_st_mean(iblock+1, sum_temp,
    // sum2_temp) << std::endl; ave_etot << istep+nstep/(nblock*10.)*iblock <<" "
    // << sum_etot/double(iblock+1) << " " << dev_st_mean(iblock+1, sum_etot,
    // sum2_etot) << std::endl;
    ave_press << istep + nstep / (nblock * 10.) * iblock << " "
              << sum_press / double(iblock + 1) << " "
              << dev_st_mean(iblock + 1, sum_press, sum2_press) << std::endl;
...
double
pbc(double r) { // Algorithm for periodic boundary conditions with side L=box
