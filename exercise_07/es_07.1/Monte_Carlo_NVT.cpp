...
int main() {
  ...
  for (int i = 1; i < ntherm; ++i) {
    ...
    if (evaluate_therm == 1) {
      Measure();
      // Print values to evaluate if system reach equilibrium
      InstantValues(i);
    }
  }
  ...
  for (int iblk = 1; iblk <= nblk; ++iblk) // Simulation
  {
    ...
    for (int istep = 1; istep <= nstep; ++istep) {
      ...
      if (instant_values == 1)
        InstantValues(istep + nstep * (iblk - 1));
    }
    if (instant_values != 1)
...
void Input(void) {
  ...
  vtail = (8.0 * pi * rho) / (9.0 * pow(rcut, 9)) -
          (8.0 * pi * rho) / (3.0 * pow(rcut, 3));
  ptail = (32.0 * pi * rho) / (9.0 * pow(rcut, 9)) -
          (16.0 * pi * rho) / (3.0 * pow(rcut, 3));
  ...
  ReadInput >> instant_values;

  ReadInput >> evaluate_therm;

  if (instant_values == 1)
    cout << "The program will print values each step, whitout averaging"
         << endl;

  if (evaluate_therm == 1)
    cout << "The program will output instant values during thermalization step"
         << endl;
...
void InstantValues(int step) {
  ...
  Epot << step << setw(wd) << walker[iv] / (double)npart + vtail << endl;
  Epot.close();
  Pres << step << setw(wd)
       << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
  Pres.close();
...
void Averages(int iblk) // Print results for current block
{
  ...
  stima_pres = rho * temp + (blk_av[iw] / blk_norm + ptail * (double)npart) /
                                vol; // Pressure
