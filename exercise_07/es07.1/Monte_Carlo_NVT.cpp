...
//Read input informations
  ...
  if(instant_values==1) cout << "The program will print values each step, whitout averaging" << endl;
  
  if(evaluate_therm==1) cout << "The program will output instant values during thermalization step" << endl;
...
void InstantValues(int step){
  ...
  Epot.close();
  Pres << step << setw(wd) << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
  Pres.close();
}
