...
int main(int argc, char *argv[]) {
  ...
  std::vector<double> r, sigma, mean_r, mean_sigma, error_r, error_sigma;
  ...
  if (output_r.is_open()) {
    for (unsigned int i = 0; i < n_blocks; i++)
      output_r << i * n_throws / n_blocks << " " << mean_r[i] - 0.5 << " "
               << error_r[i] << std::endl;
  } else
  ...
  if (output_stdev.is_open()) {
    for (unsigned int i = 0; i < n_blocks; i++)
      output_stdev << i * n_throws / n_blocks << " " << mean_sigma[i] - 1 / 12.
                   << " " << error_sigma[i] << std::endl;
  } else
    std::cerr << "PROBLEM: Unable to open output file" << std::endl;
